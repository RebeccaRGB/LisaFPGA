...
set_false_path -to [get_pins usbrst_sync_reg/D]
## False-path into the first stage of the I/O board AS synchronizer
set_false_path -to [get_pins io_board/_AS_sync_reg/D]
## False-path into the DTACK_FF_1_output register on the I/O board
## This one's a little bit odd; if you look at the Verilog source, the timing violation would seem to be on the path from AS that we just fixed
## But thanks to optimization during synthesis, a new path was created directly between the CPU board's AS and DTACK_FF, so we have to address that one too
set_false_path -to [get_pins io_board/_DTACK_FF_1_output_reg/D]
